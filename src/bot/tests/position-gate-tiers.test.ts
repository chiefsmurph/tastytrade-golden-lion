...
test("basic-only stock yes lands on the basic tier (plus boolean boost)", () => {
  // qualityToBuy makes basicStockYes true AND adds 1pt to the score → +1 boost on top of tier.
...
test("cross-account YES + strong stock YES escalates to the strong tier", () => {
  // qualityToBuy adds 1pt to score → +BOOST on top of the strong tier.
...
test("cross-account YES + basic stock YES escalates to the both tier", () => {
  // qualityToBuy adds 1pt to score → +BOOST on top of the both tier.
...
test("buyEligible=false suppresses willBuy contribution so liquidation doesn't crater score", () => {
  // With buyEligible absent: willBuy counts normally (2pts).
  const withoutFlag = gate({ ticker: "X", willBuy: true });
  assert.equal(withoutFlag.signals.goodBooleanScore, 2);

  // With buyEligible=false: willBuy is skipped entirely (0pts), score stays from health booleans only.
  const liquidating = gate({ ticker: "X", willBuy: true, buyEligible: false });
  assert.equal(liquidating.signals.goodBooleanScore, 0);

  // Health booleans still score normally even when buyEligible=false.
  const healthyWhileLiquidating = gate({
    ticker: "X",
    willBuy: true,
    buyEligible: false,
    isAboveMinSinFloor: true,
    aboveMinSis: true,
  });
  assert.equal(healthyWhileLiquidating.signals.goodBooleanScore, 2);
});

test("qualityToBuy scores 1pt regardless of buyEligible state", () => {
  const eligible = gate({ ticker: "X", qualityToBuy: true, buyEligible: true });
  assert.equal(eligible.signals.goodBooleanScore, 1);

  const liquidating = gate({ ticker: "X", qualityToBuy: true, buyEligible: false });

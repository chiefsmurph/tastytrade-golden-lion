...
export function notifyEvent(type: NotifyEventType, message: string): void {
  ...
  const sink = emitSecretLog(`${severity} [${type}] ${message}`);
  ...
  // on the secret server's stream. The `sink` flag is the emit's real outcome:
  // "sent" (delivered to an authed socket) or "queued" (held for the
  // attemptAuth ack / reconnect). Best-effort and non-throwing.
  try {
